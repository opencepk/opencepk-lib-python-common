...
(cd .. && python find_and_replace_strings_package/find_and_replace_strings/main.py --config ${TEST_DIR}/.find-and-replace.json ${TEST_DIR}/${TEST_TARGET_FILE})

#-------------------------------
# Evaluate results
#-------------------------------
echo "${TEST_SCRIPT_NAME}: Running (diff ${TEST_TARGET_FILE} ${TEST_TARGET_FILE}.expected)"
...
#-------------------------------
echo "${TEST_SCRIPT_NAME}: Resetting ${TEST_TARGET_FILE} file to original state"
